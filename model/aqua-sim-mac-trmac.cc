...
NS_LOG_COMPONENT_DEFINE("AquaSimTrMac");
NS_OBJECT_ENSURE_REGISTERED(AquaSimTrMac);
...
AquaSimTrMac::AquaSimTrMac()
{
  ...
  Simulator::Schedule(Seconds(0.1), &AquaSimTrMac::initCycle, this);
}

TypeId
AquaSimTrMac::GetTypeId()
{
  static TypeId tid = TypeId("ns3::AquaSimTrMac")
      .SetParent<AquaSimMac>()
      .AddConstructor<AquaSimTrMac>()
      .AddAttribute("PacketSize", "Size of packet",
        IntegerValue(800),
        MakeIntegerAccessor (&AquaSimTrMac::m_packetSize),
        MakeIntegerChecker<uint16_t> ())
      .AddAttribute ("StartNodeId", "ID of a starting node in TR-cycle",
        UintegerValue(0),
        MakeUintegerAccessor(&AquaSimTrMac::m_start_node_id),
        MakeUintegerChecker<uint32_t> ())
      .AddAttribute ("TotalNodes", "Total number of nodes in a swarm",
        UintegerValue(0),
        MakeUintegerAccessor(&AquaSimTrMac::m_total_nodes),
        MakeUintegerChecker<uint32_t> ())
      .AddAttribute ("GuardTime", "Interval between reception and sending the next packet",
        TimeValue(MilliSeconds(1)),
        MakeTimeAccessor(&AquaSimTrMac::m_guard_time),
        MakeTimeChecker())
      .AddAttribute ("AlgoId", "Select between different next-hop selection algorithms",
        ...
        MakeUintegerAccessor(&AquaSimTrMac::m_algo_id),
        MakeUintegerChecker<uint32_t> ())
      .AddTraceSource ("QueueSizeTrace",
                       "Trace current send-queue size",
                        MakeTraceSourceAccessor (&AquaSimTrMac::m_queue_size), "ns3::Packet::TracedCallback")
      .AddTraceSource ("E2EDelayTrace",
                       "Trace current end-to-end delay of a packet",
                        MakeTraceSourceAccessor (&AquaSimTrMac::m_e2e_delay), "ns3::Packet::TracedCallback")
    ;
  return tid;
}

int64_t
AquaSimTrMac::AssignStreams (int64_t stream)
{
...
std::vector<uint32_t>
AquaSimTrMac::runNearestNeighborTSP()
{
...
bool
AquaSimTrMac::RecvProcess (Ptr<Packet> pkt)
{
  ...
  if (m_algo_id == 0)
  {
    ...
    if (trmac_h.GetNextNodeId() == m_device->GetNode()->GetId())
    {
      ...
      {
        // initCycle();
        Simulator::Schedule(m_guard_time, &AquaSimTrMac::initCycle, this);
      }
      else
      {
        // if the sender id matches with the receiver id, then send a packet/token
        // SendPacket();
        Simulator::Schedule(m_guard_time, &AquaSimTrMac::SendPacket, this);
      }
    }
  }
  else if (m_algo_id == 1)
  ...
  if (AquaSimAddress::ConvertFrom(m_device->GetAddress()) == dst)
  {
    ...
    m_e2e_delay((Simulator::Now() - timeTag.GetTime()).GetMilliSeconds());
...
uint32_t
AquaSimTrMac::getIdbyAddress(AquaSimAddress address)
{
...
bool
AquaSimTrMac::TxProcess(Ptr<Packet> pkt)
{
  ...
  m_queue_size(m_sendQueue.size());

  return true;
}

void AquaSimTrMac::DoDispose()
{
  NS_LOG_FUNCTION(this);
