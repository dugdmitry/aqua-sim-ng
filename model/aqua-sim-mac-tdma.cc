...
AquaSimTdmaMac::GetTypeId()
{
  static TypeId tid = TypeId("ns3::AquaSimTdmaMac")
    ...
    .AddTraceSource ("QueueSizeTrace",
                      ...
                      MakeTraceSourceAccessor (&AquaSimTdmaMac::m_queue_size), "ns3::Packet::TracedCallback")
    .AddTraceSource ("E2EDelayTrace",
                      "Trace current end-to-end delay of a packet",
                      MakeTraceSourceAccessor (&AquaSimTdmaMac::m_e2e_delay), "ns3::Packet::TracedCallback")
    ;
  return tid;
}
...
AquaSimTdmaMac::TxProcess(Ptr<Packet> pkt)
{
  ...
  m_queue_size(m_send_queue.size());
...
{
  // Get current slot
  uint32_t current_slot = Simulator::Now() / (m_tdma_slot_ms + m_tdma_guard_interval_ms);
